...
class SuperheroBloc {
  ...
  void getFromFavorites() {
    ...
    getFromFavoritesSubscription = FavoriteSuperheroesStorage.getInstance()
        ...
        .listen((superhero) {
      if (superhero != null) {
        if (superheroStateSubject.valueOrNull != SuperheroPageState.loaded)
      ...
      requestSuperhero();
      //if (superhero != superheroSubject.value) replaceFavorite();
      //superheroStateSubject.add(SuperheroPageState.loaded);
    },
  ...
  void replaceFavorite() {
    ...
    replaceFavoritesSubscription = FavoriteSuperheroesStorage.getInstance()
        .replaceFavorite(superhero)
        .asStream()
        .listen((event) {
      print('Replaced in favorites: $event');
    },
            onError: (error, stackTrace) => print(
                'Error happened in replaceFavorite: $error, $stackTrace'));
  }
  ...
  void requestSuperhero() {
    ...
    requestSubscription = request().asStream().listen((superhero) {
      ...
      if (superheroStateSubject.valueOrNull != SuperheroPageState.error)
